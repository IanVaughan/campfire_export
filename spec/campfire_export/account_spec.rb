...
module CampfireExport
  describe Account do
    ...
    let(:api_token) { "test-apikey" }
    let(:account) { Account.new(subdomain, api_token) }

    let(:good_timezone) { '<?xml version="1.0" encoding="UTF-8"?>' +
       '<account>' +
    ...
    let(:bad_timezone) { good_timezone.gsub('America/Los_Angeles',
                                         'No Such Timezone') }
    ...
    context "when timezone is loaded" do
      it "determines the user's timezone" do
        account.should_receive(:get).with("/account.xml").and_return(account_xml)
